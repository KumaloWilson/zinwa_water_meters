...
export default function CreateReadingModal ({ refreshState, setIsCreateModalVisible }) {
  ...
  const onFinish = async (values) => {
    try {
      ...
      // Use the meterReadingsService to create the reading
      const response = await meterReadingsService.createMeterReading(formattedValues);
      console.log('Response:', response);
      
      message.success('Reading added successfully');
      refreshState(); // Refresh the parent component
      setIsCreateModalVisible(false); // Close the modal
      form.resetFields(); // Reset form fields
    } catch (error) {
      console.error('Error adding reading:', error);
      message.error(`Failed to add reading: ${error.message || 'Unknown error'}`);
    } finally {
      setLoading(false);
    }
  };
